...
import com.wm.edu.model.enums.ReturnPayEnum;
...
public class WechatOrderController {
    ...
    public ResultBody getReturnReason(){
        ResultBody body=new ResultBody();
        ReturnPayEnum[] returnPayEnums=ReturnPayEnum.values();
        List<String> list=new ArrayList<>();
        for (int i=0;i<returnPayEnums.length;i++){
            list.add(returnPayEnums[i].getName());
        }
        body.setData(list);
        return body;
    }
     ...
     * @return
     */
    @RequestMapping("/toReturnPay")
    public ResultBody toReturnPay(String order_id,String c_reason){
        ResultBody body=new ResultBody();
        ...
        t_refund_apply.setC_user_id(order.getC_user_id());
        Integer code=orderService.toReturnPay(order,t_refund_apply);
        body.setCode(code);
        return body;
    }
     ...
     * @return
     */
    @RequestMapping("/getRetundInfo")
    public ResultBody getRetundInfo(String order_id){
        ResultBody body=new ResultBody();
        T_order t_order=orderService.getOrderRetrunApply(order_id);
        body.setData(t_order);
        return body;
    }
