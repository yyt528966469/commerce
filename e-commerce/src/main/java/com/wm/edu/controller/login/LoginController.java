...
public class LoginController {
   ...
   /* @RequestMapping("/")
    ...
    public String echats(Model model){
        ...
        T_commodity_info t_commodity_info=new T_commodity_info();
        t_commodity_info.setRows(6);
        Page<T_commodity_info> page=commodityInfoService.getInfoByComm_num(t_commodity_info);
        C_user_vip c_user_vip=new C_user_vip();
        c_user_vip.setRows(4);
        c_user_vip.setSord("desc");
        c_user_vip.setSidx("create_time");
        Page<C_user_vip> page1=userVipService.getVipPage(c_user_vip);
        model.addAttribute("commodityList",page.getResult());
        model.addAttribute("vipList",page1.getResult());
        model.addAttribute("user_num",user_num);
        model.addAttribute("today_order",today_order);
        return "index/index";
    }
    @RequestMapping("/loadOrderTypeNum")
    @ResponseBody
    public Map<String,Object> loadOrderTypeNum(){
        Map<String,Object> map=new HashMap<>();
        List<Integer> list=orderService.getOrderTypeNum();
        List<Integer> otherNum=orderService.getOtherOrderNum();
        map.put("numList",list);
        map.put("otherOrderNum",otherNum);
        return map;
    }
