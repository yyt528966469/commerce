...
public class OrderService {
    ...
    @Autowired
    private T_refund_applyMapper t_refund_applyMapper;
    ...
    @Transactional
    public Integer toReturnPay(T_order t_order,T_refund_apply t_refund_apply){
        Integer code=200;
        try {
            t_orderMapper.updateSelectiveById(t_order);
            /**
             *
             */
            //T_refund_apply t_refund_apply=new T_refund_apply();
            t_refund_apply.setId(AttaUtils.getNo());
            t_refund_apply.setCreate_time(new Date());
            //t_refund_apply.setC_reason(c_reason);
            t_refund_apply.setOrder_id(t_order.getOrder_id());
            t_refund_apply.setStatus(0);
            t_refund_applyMapper.insertSelective(t_refund_apply);
        }catch (Exception e){
            e.printStackTrace();
            code=502;
    ...
    public Map<String,Object> confirmShopOrder(String user_id){
        ...
        map.put("sum_price",df.format(sum_price.floatValue()));
        map.put("t_coupons",t_coupons);
        C_user_vip c_user_vip=getPayType(user_id);
        map.put("c_user_vip",c_user_vip);
        return map;
    }


    private C_user_vip getPayType(String user_id){
        ;//微信支付
        C_user_vip c_user_vip=userVipService.getVipByUserId(user_id);
    ...
    @Transactional
    public Integer balancePay(String order_id){
        Integer code=200;
        try {
            T_order t_order=getOrdermin(order_id);
            if(t_order!=null){
                C_user_vip c_user_vip=userVipService.getVipByUserId(t_order.getC_user_id());
                if(c_user_vip!=null){
                    ...
                    if(balance>real_price){
                        //price.subtract()
                        price=price.subtract(new BigDecimal(real_price));
                        ...
                        record.setPrice(t_order.getReal_price());
                        record.setC_user_id(t_order.getC_user_id());
                        record.setCreate_time(new Date());
                        if(t_order.getEat_type()==0){
                            record.setContent("外卖点餐");
                        }else if(t_order.getEat_type()==1){
                            record.setContent("到店自取");
                        }else if(t_order.getEat_type()==2){
                            record.setContent("店内点餐");
                        }else if(t_order.getEat_type()==3){
                            record.setContent("购物");
                        }
                        paymentRecordService.insertRecord(record);
                        ...
                        order.setOrder_id(t_order.getOrder_id());
                        t_orderMapper.updateSelectiveById(order);
                    }else {
                        code=501;
                    }
     ...
     */
    @Transactional
    public Integer returnPay(T_order t_order){
        Integer code=200;
        try {
            Float real_price=t_order.getReal_price();
            C_user_vip c_user_vip=userVipService.getVipByUserId(t_order.getC_user_id());
            Float balance=c_user_vip.getBalance();
            BigDecimal price=new BigDecimal(balance);
            price=price.add(new BigDecimal(real_price));
            C_user_vip user_vip=new C_user_vip();
            user_vip.setId(c_user_vip.getId());
            user_vip.setBalance(price.floatValue());
            userVipService.saveUserVip(user_vip);
            ...
            order.setStatus(9);
            t_orderMapper.updateSelectiveById(order);
            String coupons_id=t_order.getCoupons_id();
            T_user_coupons t_user_coupons=new T_user_coupons();
            t_user_coupons.setStatus(0);
            t_user_coupons.setCoupons_id(coupons_id);
            t_user_coupons.setC_user_id(t_order.getC_user_id());
            userCouponsService.updateUserCouspons(t_user_coupons);//退回优惠劵
            //couponsService.saveCoupons();
        }catch (Exception e){
            e.printStackTrace();
            code=502;
            throw new RuntimeException();
     ...
     * @return
     */
    public String returnMoney(String order_id){
        String res="0";
        try {
            T_order t_order=t_orderMapper.getOrdermin(order_id);
            if(t_order.getPay_type()==0){
                String reqParams =PayUtils.refundPay(t_order);//执行退款操作
                Map map = PayUtils.doXMLParse(reqParams);
                String return_code = (String) map.get("return_code");//返回状态码
                if (return_code.equals("SUCCESS")) {
                   /* T_order order=new T_order();
    ...
    public T_order getOrderRetrunApply(String order_id){
        return t_orderMapper.getOrderRetrunApply(order_id);
    }

    /**
     *
     * @param t_refund_apply
     */
    @Transactional
    public String  submitApplyInfo(T_refund_apply t_refund_apply){
        String res="0";
        try {
            t_refund_apply.setSh_time(new Date());
            //t_refund_apply.set
            t_refund_applyMapper.updateByPrimaryKeySelective(t_refund_apply);
            T_order t_order=new T_order();
            t_order.setOrder_id(t_refund_apply.getOrder_id());
            if(t_refund_apply.getStatus()==2){
                t_order.setStatus(10);
                t_order.setOver_time(new Date());
            }else {
                t_order.setStatus(9);
                t_order.setOver_time(new Date());
     ...
     * @return
     */
    public List<Integer> getOrderTypeNum(){
        return t_orderMapper.getOrderTypeNum();
    }

    public List<Integer> getOtherOrderNum(){
        return t_orderMapper.getOtherOrderNum();
    }
