...
<script>
    $(document).ready(function () {
        ...
        $("#table_list").jqGrid({
            ...
            rowList: [10, 20, 30],
            root:"rows",
            colNames: ['ID','公告内容','顺序','状态','创建时间','操作'],
            colModel: [
                {
                    name: 'notice_id',
                    index: 'notice_id',
                ...
                },
                {
                    ...
                    formatter: function(cellvalue,options,rowObject) {
                        ...
                        var notice_id =rowObject.notice_id;
                        var status=rowObject.status;
                            var v_result = "";
                            v_result += "<a  class='a_click' style='width:65px;border:none; cursor: pointer;'  onClick='toAddOrEdit(\"" + notice_id + "\");'> 编辑</a>";
                        v_result += " &nbsp;&nbsp; <a  class='a_click' style='width:65px;border:none; cursor: pointer;'  onClick='deleteNotice(\"" + notice_id + "\");'> 删除</a>";
                        if (status==0){
                            v_result += "&nbsp;&nbsp; <a  class='a_click' style='width:65px;border:none; cursor: pointer;'  onClick='updateStatus(\"" + notice_id + "\",\"1\");'> 上架</a>";
                        }else {
                            v_result += "&nbsp;&nbsp; <a  class='a_click' style='width:65px;border:none; cursor: pointer;'  onClick='updateStatus(\"" + notice_id + "\",\"0\");'> 下架</a>";
                        }
   ...
   /* $("#searchBtn").click(function(){
    ...
    function toAddOrEdit(notice_id) {
        //if(activit_id)
        //$("#myiframe").load("sysuser/editUser?user_id="+user_id);
        window.location.href="../notice/toAddOrEdit?notice_id="+notice_id;
    }
    
    function updateStatus(notice_id,status) {
        var title="上架";
        if(status==0){
            title="下架";
        }
