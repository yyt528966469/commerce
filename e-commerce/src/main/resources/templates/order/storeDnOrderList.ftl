  ...
  <div class="col-xs-12" >
    <div class="box-header">
      ...
      <ul id="status_ul" class="nav nav-tabs" style="border-bottom: 0px solid #e7eaec;">
          <li ><a data-toggle="tab" onclick="searchStatus(null)"
          ...
          <#list orderStauteEnums as orderStaute>
               <li <#if orderStaute.index==1>class="active"</#if> ><a data-toggle="tab" onclick="searchStatus(${orderStaute.index})"
                                      tabindex='1'>${orderStaute.getName(orderStaute.index)}</a></li>
          </#list>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#table_list").jqGrid({
            ...
            rowList: [10, 20, 30],
            root:"rows",
            colNames: ['ID','状态','订单编号','金额','桌号','人数','状态','创建时间','操作'],
            colModel: [
                {
                    name: 'order_id',
                    index: 'order_id',
                ...
                {
                    name: 't_order_scavenging.table_num',
                    index: 't_order_scavenging.table_num',
                    align : "center",
                    width: 13
                },
                {
                    name: 't_order_scavenging.person_num',
                    index: 't_order_scavenging.person_num',
                    align : "center",
                    width: 13
                },
                {
    ...
    $("#searchBtn").click(function(){
        ...
        $("#table_list").jqGrid('setGridParam',{
            url:"../order/list",
            postData:{order_id:order_id,eat_type:2,stauts:v_status}, //发送数据
            page:1
    ...
    function searchStatus(status) {
        ...
        $("#table_list").jqGrid('setGridParam',{
            url:"../order/list",
            postData:{status:status,eat_type:2}, //发送数据
            page:1
        }).trigger("reloadGrid"); //重新载入
    }
