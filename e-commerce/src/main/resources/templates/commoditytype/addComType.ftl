  ...
  <div class="col-xs-12">
    ...
    <div class="box-body">
        <form id="myform">
            ...
            <input type="hidden" name="type_id" value="${commodity_type.type_id!''}"/>
      <div class="box-line">
        <span class="title mid-inline-block">类型名称：</span>
        <input class="long"  maxlength="10" aria-required="true" required=""  name="type_name"  value="${commodity_type.type_name!'' }" placeholder="请输入类型名"/><span style="color: red" id="nameInfo"></span>
      </div>
      <div class="box-line">
        <span class="title mid-inline-block">顺序号：</span>
        <input class="long" maxlength="3" aria-required="true" required="" name="sort" value="${commodity_type.sort!'' }" placeholder="请输入顺序号"/>
      </div>

            <div class="box-line">
                <span class="title mid-inline-block">类型图片：</span>
                <label for="cover" style="width: 25%;">
                    <input id="image_file" name="image_file" dir=rtl class="file-loading" type="file" >
                    <input type="hidden" id="img" name="img" value="${commodity_type.img!''}">
                </label>
...
<script>
            ...
            success:function(data){
                if(data.result=='0'){
                    layer.alert("保存成功",function (index) {
                        window.location.href="../commoditytype/main";
                    });
                }else {
                    return false;
                }
    ...
    var v_img_url = '${commodity_type.img!""}';
