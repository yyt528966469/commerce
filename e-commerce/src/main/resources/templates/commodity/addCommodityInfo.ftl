  ...
  <div class="col-xs-12">
    ...
    <div class="box-body">
        <form id="myform">
            ...
            <input type="hidden" name="commodity_id" value="${commodity_info.commodity_id!''}"/>
            <div class="box-line">
                <span class="title mid-inline-block">商品类型：</span>
                <select name="commodity_type">
                    <option>==请选择==</option>
                    <#if commodity_info.commodity_type?? >
                        <#list typeList as type>
                        <option value="${type.type_id}" <#if commodity_info.commodity_type==type.type_id >selected="selected"</#if> >${type.type_name}</option>
                        </#list>
                        <#else >
                            <#list typeList as type>
      ...
      <div class="box-line">
        ...
        <input class="long"  maxlength="20" aria-required="true" required=""  name="commodity_name"  value="${commodity_info.commodity_name!'' }" placeholder="请输入商品名称"/><span style="color: red" id="nameInfo"></span>
      </div>
      <div class="box-line">
        <span class="title mid-inline-block">商品简介：</span>
        <input class="long" maxlength="300" aria-required="true" required="" name="commodity_introduction" value="${commodity_info.commodity_introduction!'' }" placeholder="请输入商品简介"/>
      </div>
            <div class="box-line">
                <span class="title mid-inline-block">商品价格：</span>
                <input class="long" maxlength="7" aria-required="true" required="" name="price" value="${commodity_info.price!'' }" placeholder="请输入商品价格"/>
            </div>
            <div class="box-line">
                <span class="title mid-inline-block">是否推荐：</span>
                <input style="height:13px;" type="radio" onclick="changeRan()" value="1" <#if commodity_info.is_recommend??> <#if commodity_info.is_recommend==1 >checked</#if></#if> name="is_recommend"> 是 &nbsp;&nbsp;&nbsp; <input style="height:13px;" <#if commodity_info.is_recommend??><#if commodity_info.is_recommend!=1 >checked</#if><#else >checked</#if> onclick="changeRan()" value="0" type="radio"  name="is_recommend"> 否
            </div>
            <div id="weight_div" class="box-line">
                <span class="title mid-inline-block">推荐权重：</span>
                <input class="long" maxlength="3"  name="weight" value="${commodity_info.weight!'' }" placeholder="请输入权重"/>
            </div>
            <div class="box-line" >
                ...
                <label for="cover" style="width: 25%;">
                    ...
                    <input type="hidden" id="img_src" name="img_src" value="${commodity_info.img_src!''}">
...
<script>
            ...
            success:function(data){
                if(data.result=='0'){
                    layer.alert("保存成功",function (index) {
                        layer.close(index);
                        window.location.href="../commodity/main";
                    });
    ...
    $(document).ready(function () {
        ...
        var uploadImagePath = "../commodity/uploadImages";
    ...
    var v_img_url = '${commodity_info.img_src!""}';
    ...
    function initImageInput(inputId,uploadUrl) {
        ...
        control.fileinput({
            ...
            var head_arry=[];
            var text_arry=[];
                     <#if commodity_info.photoList??>
                         <#list  commodity_info.photoList as photo>
            ...
            $("#photo_head").fileinput({
                ...
                uploadUrl : "../commodity/uploadHead",
                showUpload: false, //是否显示上传按钮
                showRemove: true,  //是否显示移除按钮
                //showPreview :true, //是否显示预览
                showCaption: true,//是否显示文件标题。默认为true。
            ...
            $("#photo_text").fileinput({
                ...
                uploadUrl : "../commodity/uploadText",
                showUpload: false, //是否显示上传按钮
                showRemove: true,  //是否显示移除按钮
                //showPreview :true, //是否显示预览
                showCaption: true,//是否显示文件标题。默认为true。
